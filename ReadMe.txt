=== subscribe2-IT ===
Contributors: MattyRob, Skippy, RavanH, Giovanni Siani
...
subcribe2 ora tadotto in italiano da Giovanni Siani e migliorato nell'interfaccia utente.

== Description ==

subscribe2-IT provides a comprehensive subscription management and email notification system for WordPress blogs that sends email notifications to a list of subscribers when you publish new content to your blog.
...
The format of the email can also be customised for per-post notifications, subscribe2-IT can generate emails for each of the following formats:
...
If you want to send full content HTML emails to Public Subscribers too then upgrade to [subscribe2-IT HTML](http://wpplugins.com/plugin/46/subscribe2-IT-html).
...
2. Search for subscribe2-IT, click "Install Now" and then Activate the Plugin
3. Click the "Settings" admin menu link, and select "subscribe2-IT".
...
7. Create a [WordPress Page](http://codex.wordpress.org/Pages) to display the subscription form.  When creating the page, you may click the "S2" button on the QuickBar to automatically insert the subscribe2-IT token.  Or, if you prefer, you may manually insert the subscribe2-IT shortcode or token: [subscribe2-IT] or the HTML invisible `<!--subscribe2-IT-->` ***Ensure the token is on a line by itself and that it has a blank line above and below.***
...
8. In the WordPress "Settings" area for subscribe2-IT select the page name in the "Appearance" section that of the WordPress page created in step 7.

MANUAL INSTALLATION

1. Copy the entire /subscribe2-IT/ directory into your /wp-content/plugins/ directory.
...
You need to pay for the [subscribe2-IT HTML version](http://wpplugins.com/plugin/46/subscribe2-IT-html).
...
Next you could search in the [WordPress forums](http://wordpress.org/support/), the old [subscribe2-IT Forum](http://getsatisfaction.com/subscribe2-IT/), or the [subscribe2-IT blog FAQs](http://subscribe2-IT.wordpress.com/category/faq/).
...
A comprehensive guide that covers many, if not all, of the subscribe2-IT features is available to purchase from the [My WP Works](http://mywpworks.com/store/subscribe2-IT-ebook/)

= Sending post notifications or email with subscribe2-IT =

subscribe2-IT sends an email at the very moment the post is published. Since subscribe2-IT sends live mail with no un-do, it's important to use the Preview function in WordPress to make sure the post has been edited to perfection *before* moving it from Draft to Published mode.
...
While the template field in Settings | subscribe2-IT does not display HTML by default, feel free to add HTML to it as needed. You can insert references to static images for use as banners, wrap sections of the template in divs or other elements, or do whatever you like.
...
subscribe2-IT provides a facility to work around a restriction of the maximum number of recipients per email by sending batches of emails.  To enable this feature, go to Settings->subscribe2-IT and located the setting to restrict the number of recipients per email. If this is set to 30 then each outgoing email notification will only contain addresses for 30 recipients.

Reminder: because subscribe2-IT places all recipients in BCC fields, and places the blog admin in the TO field, the blog admin will receive one email per batched delivery.  So if you have 90 subscribers, the blog admin should receive three post notification emails, one for each set of 30 BCC recipients.
...
**Restrictions on the number of messages sent**. In order to combat spam many hosts are now implementing time based limitations. This means you are only allowed to send a certain number of messages per unit time, 500 per hour for example. subscribe2-IT does not have a work around for this inbuilt but see the next question.

= My host has a limit of X emails per hour / day, can I limit the way subscribe2-IT sends emails? =
...
If the above has not put you off then I spent some time writing a Mail Queue script for subscribe2-IT that adds the mails to a database table and sends then in periodic batches. It is available, at a price, [here](http://wpplugins.com/plugin/76/wordpress-mail-queue-wpmq).
...
subscribe2-IT supports 4 potential email formats for Susbcribers so you will get a preview for each of the different possibilities.

= Why do I need to create a WordPress Page =

subscribe2-IT uses a filter system to display dynamic output to your readers. The token may result in the display of the subscription form, a subscription message, confirmation that an email has been sent, a prompt to log in. This information needs a static location for the output of the filter and a WordPress page is the ideal place for this to happen.

If you decide to use subscribe2-IT only using the widget you must still have at least one WordPress page on your site for subscribe2-IT to work correctly.

= Why is my admin address getting emails from subscribe2-IT? =
...
subscribe2-IT creates four (4) new admin menus in the back end of WordPress. These are all under the top level menu header **subscribe2-IT**.
...
**Note:** In versions of the plugin prior to version 7.0 the menus are under the WordPress system at Posts -> Mail Subscribers, Tools -> Subscribers, Users -> Subscriptions and Settings -> subscribe2-IT.
...
Registered Users have registered with your WordPress blog (provided you have enabled this in the core WordPress settings). Registered users of the blog can elect to receive email notifications for specific categories (unless Digest email are select, then it is an opt in or out decision).  The Users->Subscription menu item will also allow them greater control to select the delivery format (plaintext or HTML), amount of message (excerpt or full post), and the categories to which they want to subscribe.  You, the blog owner, have the option (Options->subscribe2-IT) to allow registered users to subscribe to your excluded categories or not.

**Note** You can send HTML emails to Public Subscribers with the paid [subscribe2-IT HTML version](http://wpplugins.com/plugin/46/subscribe2-IT-html) of the plugin.

= Can I put the form elsewhere? (header, footer, sidebar without the widget) =

The simple answer is yes you can but this is not supported so you need to figure out any problems that are caused by doing this on your own. Read <a href="http://subscribe2-IT.wordpress.com/2006/09/19/sidebar-without-a-widget/">here</a> for the basic approach.

= I'd like to be able to collect more information from users when they subscribe, can I? =

Get them to register with your blog rather than using the subscribe2-IT form. Additional fields would require much more intensive form processing, checking and entry into the database and since you won't then be able to easily use this information to personalise emails there really isn't any point in collecting this data.

= How do I use the subscribe2-IT shortcode? =

In version 6.1 of subscribe2-IT the new standard WordPress shortcode [subscribe2-IT] was introduced. By default, it behaves same as old subscribe2-IT token, `<--subscribe2-IT-->`, which means that it will show the same subscribe2-IT output in your chosen page in WordPress or in the Widget.
...
* In the subscribe2-IT email template for new post, add text "You can unsubscribe on a following page:" which is followed with link to "Unsubscribe" page

= I can't find or insert the subscribe2-IT token or shortcode, help! =

If, for some reason the subscribe2-IT button does not appear in your browser window try refreshing your browser and cache (Shift and Reload in Firefox). If this still fails then insert the token manually. In the Rich Text Editor (TinyMCE) make sure you switch to the "code" view and type in [subscribe2-IT] or <!--subscribe2-IT-->.
...
If you opt for the latter way look in the options table for the subscribe2-IT_options settings (it's an array) and you'll need to change the 'last_s2cron' value to a timestamp for last week. Then force the cron event to run again with [WP-Crontrol](http://wordpress.org/extend/plugins/wp-crontrol/).

= I would really like Registered users to have the Subscription page themed like my site, is this possible? =

Yes, it is. There is a small extension to subscribe2-IT that delivers exactly this functionality. It is available from [Theme Tailors](http://stiofan.themetailors.com/store/products/tt-subscribe2-IT-front-end-plugin/) for just $2.
...
For a WordPress page you use the normal subscribe2-IT token but add a 'link' parameter with the text you'd like your users to click, so something like:

`[subscribe2-IT link="Click Here to Subscribe"]`

For Sidebar users, visit the Widgets page and look in the subscribe2-IT Widget, there is a new option at the bottom called "Show as link". If you choose this a link will be placed in your sidebar that displays the form when clicked.
...
= I want to change the kinds of users who can access the subscribe2-IT menus. Is that possible? =

Yes, it is possible with a little bit for code either in a custom plugin or your functions.php file in your theme. You use the add_filter() command that is part of WordPress to change the [capability](http://codex.wordpress.org/Roles_and_Capabilities#Capabilities) that allows access to each of the subscribe2-IT menus.
...
By default Public Subscribers get plain text emails and only Registered Subscribers can opt to receive email in HTML format. If you really want HTML for all you need to pay for the [subscribe2-IT HTML version](http://wpplugins.com/plugin/46/subscribe2-IT-html).

= Which version should I be using, I'm on WordPress x.x.x? =

WordPress 3.1 and up requires subscribe2-IT from the 7.x or 8.x stable branch. The most recent version is hosted via [Wordpress.org](http://wordpress.org/extend/plugins/subscribe2-IT/).

WordPress 2.8 and up requires subscribe2-IT from the 6.x stable branch. The most q version is [6.5](http://downloads.wordpress.org/plugin/subscribe2-IT.6.5.zip).

WordPress 2.3.x through to 2.7.x require subscribe2-IT from the 4.x or 5.x stable branch. The most recent version is [5.9](http://downloads.wordpress.org/plugin/subscribe2-IT.5.9.zip).

WordPress 2.1.x and 2.2.x require subscribe2-IT from the 3.x stable branch. The most recent version is [3.8](http://downloads.wordpress.org/plugin/subscribe2-IT.3.8.zip).

WordPress 2.0.x requires subscribe2-IT from the 2.x stable branch. The most recent version is [2.22](http://downloads.wordpress.org/plugin/subscribe2-IT.2.22.zip).
...
Secondly, make sure that the token ([subscribe2-IT] or <!--subscribe2-IT-->) is correctly entered in your page with a blank line above and below. The easiest way to do this is to deactivate the plugin, visit your WordPress page and view the source. The token should be contained in the source code of the page. If it is not there you either have not correctly entered the token or you have another plugin that is stripping the token from the page code.
...
4. The Subsribe2->subscribe2-IT admin page generated by the plugin.
...
* Fixed subscribe2-IT implementation of custom taxonomies
...
* Updated screenshots to subscribe2-IT 8.0 and WordPress 3.3.1 screen layouts
...
* Implemented a change so that categories are not displayed in the filter dropdowns when using subscribe2-IT in digest mode
...
* Improved the TinyMCE plugin code that handles placement of the subscribe2-IT shortcode in the rich text editor
...
* NOTE subscribe2-IT now requires WordPress 3.1 or higher
* Implemented top level menu to make subscribe2-IT menu access easier
...
* Show user name for Registered Users when mouse is hovered over email address in subscribe2-IT->Subscribers
...
* Added warning on the Options page to avoid using the subscribe2-IT KEYWORDS in posts as they will get substituted
* Added HTML LABEL tags to the subscribe2-IT form to support WCAG
...
* Fixed a glitch in the HTML tags in the subscribe2-IT Widget that affected drag and drop functions in WordPress - thanks to Marty McOmber
...
* Introduced a more flexible subscribe2-IT shortcode - thanks to Milan for the patch code

= Version 6.0 by Matthew Robinson =

* Improved case sensitive SQL queries to avoid issues on some server configurations
* Introduced option to check the notification override button by default at Settings->subscribe2-IT - Appearance
...
* Removed several functions from the code for WordPress versions prior to 2.8 (2.8 is now a minimum requirement for subscribe2-IT)
...
* Fixed issue where successful emailing from Post->Mail Subscribers would report as failed if subscribe2-IT is set to email one user per email - Thanks to Meini from Utech Computer Solutions (www.utechworld.com)
...
* Ensure that subscribe2-IT sanitises email addresses to RFC5322 and RFC5321 - Thanks to Vital
...
* Issue an error message in Settings->subscribe2-IT if there is no WordPress page published on the blog site
* Improved usage of time dropdown in Settings->subscribe2-IT so it works for periodic emails that are on a less than daily frequency
...
* Fixed an issue where Registered Users who have requests a password reset disappear from subscribe2-IT
...
* Added screen_icon() to each subscribe2-IT admin page
...
* Use PHP variable for subscribe2-IT folder to allow for easier renaming (if needed)
...
* Restored several FAQs to the ReadMe file and the [WordPress.org FAQ section](http://wordpress.org/extend/plugins/subscribe2-IT/faq/)

= Version 5.1 by Matthew Robinson =

* Add widget options to add custom text before and a after the dynamic subscribe2-IT output - thank to Lee Willis
* Add protection against SQL injection attacks to the data entered into the subscribe2-IT table
...
* Added links to online subscribe2-IT resources into the Options->Subsribe2 page
...
* Added ability to send email previews to currently logged in user from Settings->subscribe2-IT
...
* Correct minor layout issue in Settings->subscribe2-IT
...
* Added ability to define email Subject contents in Settings->subscribe2-IT
...
* Improve layout in the subscribe2-IT form
...
* Improved compatibility with other plugins by only inserting JavaScript code into subscribe2-IT's own admin pages
...
* Full XHTML compliance on all subscribe2-IT admin pages
* Decode HTML entity codes in notification email subjects
* Added subscribe2-IT support for blogging via email
...
* Fixed some issues with the email headers now that subscribe2-IT is using wp_mail() again
...
* Fixed issue where subscribe2-IT grabbed the first page from the database even if it wasn't published
...
* Fixed time display for cron jobs in Options->subscribe2-IT when displayed on blogs using a time offset
* Added Debug plugin to the download package
* Improved descriptions of email template keywords in Options->subscribe2-IT
...
* Added an Un-installer to completely removed subscribe2-IT from your WordPress install
...
* Ensured all admin pages created by subscribe2-IT are valid XHTML 1.0 Transitional code
...
* Improved handling of the subscribe2-IT option array
...
* Added functionality to allow for subscribe2-IT Sidebar Widget
...
* By Scott Merrill, see http://www.skippy.net/blog/category/wordpress/plugins/subscribe2-IT/
