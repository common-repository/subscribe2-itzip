...
if ( isset($_POST['s2_admin']) && 'mail' == $_POST['s2_admin'] ) {
	...
	if ( $status ) {
		if ( isset($_POST['preview']) ) {
			$message = "<p class=\"s2_message\">" . __('Messaggio di anteprima inviato!', 'subscribe2') . "</p>";
		} else {
			$message = $this->mail_sent;
		}
	} elseif ( empty($body) ) {
		$message = __('La tua emai era vuota', 'subscribe2');
	} else {
		global $phpmailer;
...
echo "<h2>" . __('Invia una email agli iscritti', 'subscribe2') . "</h2>\r\n";
...
if ( isset($_POST['subject']) ) {
	...
	$subject = __('Un messaggio da', 'subscribe2') . " " . html_entity_decode(get_option('blogname'), ENT_QUOTES);
}
if ( !isset($_POST['content']) ) {
	$body = '';
}
echo "<p>" . __('Oggetto', 'subscribe2') . ": <input type=\"text\" size=\"69\" name=\"subject\" value=\"" . $subject . "\" /> <br /><br />";
echo "<textarea rows=\"12\" cols=\"75\" name=\"content\">" . $body . "</textarea>";
echo "<br /><br />\r\n";
echo __('Riceventi:', 'subscribe2') . " ";
$this->display_subscriber_dropdown('registered', false, array('all'));
echo "<input type=\"hidden\" name=\"s2_admin\" value=\"mail\" />";
echo "</p>";
echo "<p class=\"submit\"><input type=\"submit\" class=\"button-secondary\" name=\"preview\" value=\""  . __('Anteprima', 'subscribe2') . "\" /><input type=\"submit\" class=\"button-primary\" name=\"send\" value=\"" . __('Invia', 'subscribe2') . "\" /></p>";
echo "</form></div>\r\n";
