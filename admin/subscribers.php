	...
	if ( !empty($_POST['addresses']) ) {
		...
		foreach ( preg_split ("|[\s,]+|", $_POST['addresses']) as $email ) {
			...
			if ( is_email($email) && $_POST['subscribe'] ) {
				...
				$this->add($email, true);
				$message = "<div id=\"message\" class=\"updated fade\"><p><strong>" . __('Indirizzo(i) iscritti!', 'subscribe2') . "</strong></p></div>";
				...
				$this->delete($email);
				$message = "<div id=\"message\" class=\"updated fade\"><p><strong>" . __('Address(i) disinscritti!', 'subscribe2') . "</strong></p></div>";
			}
		}
		if ( $sub_error != '' ) {
			echo "<div id=\"message\" class=\"error\"><p><strong>" . __('Alcune emails non furono eleborate, la seguente era già iscritta' , 'subscribe2') . ":<br />$sub_error</strong></p></div>";
		}
		if ( $unsub_error != '' ) {
			echo "<div id=\"message\" class=\"error\"><p><strong>" . __('Alcune emails non furono eleborate, la seguente non era nel database' , 'subscribe2') . ":<br />$unsub_error</strong></p></div>";
		}
		echo $message;
		...
		if ( isset($_POST['delete']) ) {
			...
			echo "<div id=\"message\" class=\"updated fade\"><p><strong>" . __('Indirizzo(i) cancellato!', 'subscribe2') . "</strong></p></div>";
		}
		if ( isset($_POST['confirm']) ) {
			foreach ( $_POST['confirm'] as $address ) {
				$this->toggle($this->sanitize_email($address));
			}
			$message = "<div id=\"message\" class=\"updated fade\"><p><strong>" . __('Stato cambiato!', 'subscribe2') . "</strong></p></div>";
		}
		...
		echo "<div id=\"message\" class=\"updated fade\"><p><strong>" . __('Email(s) di promemoria inviate!', 'subscribe2') . "</strong></p></div>";
	} elseif ( isset($_POST['sub_categories']) && 'subscribe' == $_POST['manage'] ) {
		$this->subscribe_registered_users($_POST['exportcsv'], $_POST['category']);
		echo "<div id=\"message\" class=\"updated fade\"><p><strong>" . __('Utenti registrati iscritti!', 'subscribe2') . "</strong></p></div>";
		...
		echo "<div id=\"message\" class=\"updated fade\"><p><strong>" . __('Utenti registrati disinscritti!', 'subscribe2') . "</strong></p></div>";
	} elseif ( isset($_POST['sub_format']) ) {
		$this->format_change( $_POST['exportcsv'], $_POST['format'] );
		echo "<div id=\"message\" class=\"updated fade\"><p><strong>" . __('Format aggiornato per gli utenti registrati selezionati!', 'subscribe2') . "</strong></p></div>";
	} elseif ( isset($_POST['sub_digest']) ) {
		$this->digest_change( $_POST['exportcsv'], $_POST['sub_category'] );
		echo "<div id=\"message\" class=\"updated fade\"><p><strong>" . __('Riassunto di iscrizione aggiornato per gli utenti registrati selezionati!', 'subscribe2') . "</strong></p></div>";
	}
}
...
if ( !empty($subscribers) ) {
	...
	if ( $page > 1 ) {
		$args['s2page'] = $page - 1;
		$strip .= '<a class="prev" href="' . esc_url(add_query_arg($args)) . '">&laquo; '. __('Pagina precedente', 'subscribe2') .'</a>' . "\n";
	}
	if ( $total_pages > 1 ) {
	...
	if ( ( $page ) * $this->subscribe2_options['entries'] < $total_subscribers ) {
		$args['s2page'] = $page + 1;
		$strip .= "<a class=\"next\" href=\"" . esc_url(add_query_arg($args)) . "\">". __('Pagina successiva', 'subscribe2') . " &raquo;</a>\n";
	}
}
...
echo "<h2>" . __('Gestisci gli iscitti', 'subscribe2') . "</h2>\r\n";
echo "<form method=\"post\">\r\n";
if ( function_exists('wp_nonce_field') ) {
	wp_nonce_field('subscribe2-manage_subscribers' . $s2nonce);
}
echo "<div class=\"s2_admin\" id=\"s2_add_subscribers\">\r\n";
echo "<h2>" . __('Aggiungi/Rimuovi iscritti', 'subscribe2') . "</h2>\r\n";
echo "<p>" . __('Inserisci indirizzi, uno per linea o separati da virgola', 'subscribe2') . "<br />\r\n";
echo "<textarea rows=\"2\" cols=\"80\" name=\"addresses\"></textarea></p>\r\n";
echo "<input type=\"hidden\" name=\"s2_admin\" />\r\n";
echo "<p class=\"submit\" style=\"border-top: none;\"><input type=\"submit\" class=\"button-primary\" name=\"subscribe\" value=\"" . __('Iscriviti', 'subscribe2') . "\" />";
echo "&nbsp;<input type=\"submit\" class=\"button-primary\" name=\"unsubscribe\" value=\"" . __('Disinscriviti', 'subscribe2') . "\" /></p>\r\n";
echo "</div>\r\n";

// subscriber lists
echo "<div class=\"s2_admin\" id=\"s2_current_subscribers\">\r\n";
echo "<h2>" . __('Iscritti attuali', 'subscribe2') . "</h2>\r\n";
echo "<br />";
$this->display_subscriber_dropdown($what, __('Filtra', 'subscribe2'));
echo "<br /><br />";
...
echo "<td><input type=\"submit\" class=\"button-secondary\" name=\"search\" value=\"" . __('Cerca iscritti', 'subscribe2') . "\" /></td>\r\n";
if ( $reminderform ) {
	echo "<td style=\"width: 25%; text-align: right;\"><input type=\"hidden\" name=\"reminderemails\" value=\"" . $reminderemails . "\" />\r\n";
	echo "<input type=\"submit\" class=\"button-secondary\" name=\"remind\" value=\"" . __('Invia email di promemoria', 'subscribe2') . "\" /></td>\r\n";
} else {
...
if ( !empty($subscribers) ) {
	...
	echo "<input type=\"submit\" class=\"button-secondary\" name=\"csv\" value=\"" . __('Salva le emails in un file CSV', 'subscribe2') . "\" /></td>\r\n";
} else {
	echo "<td style=\"width: 25%;\"></td>";
}
echo "</tr>";
...
if ( !empty($subscribers) ) {
	echo "<tr><td colspan=\"3\" style=\"text-align: center;\"><input type=\"submit\" class=\"button-secondary\" name=\"process\" value=\"" . __('Processa', 'subscribe2') . "\" /></td>\r\n";
...
if ( !empty($subscribers) ) {
	...
	echo "<img src=\"" . $urlpath . "include/accept.png\" alt=\"&lt;\" title=\"" . __('Conferma questo indirizzo email', 'subscribe2') . "\" /></td>\r\n";
	echo "<td style=\"width: 4%; text-align: center;\">";
	echo "<img src=\"" . $urlpath . "include/exclamation.png\" alt=\"&gt;\" title=\"" . __('Non confermare questo indirizzo email', 'subscribe2') . "\" /></td>\r\n";
	echo "<td style=\"width: 4%; text-align: center;\">";
	echo "<img src=\"" . $urlpath . "include/cross.png\" alt=\"X\" title=\"" . __('Cancella questo indirizzo email', 'subscribe2') . "\" /></td><td colspan=\"3\"></td></tr>\r\n";
	...
	echo "<td colspan =\"3\" style=\"text-align: left;\"><strong>" . __('Seleziona / Deseleziona tutto', 'subscribe2') . "</strong></td></tr>\r\n";
	...
	foreach ( $subscribers as $subscriber ) {
		...
		if ( in_array($subscriber, $confirmed) ) {
			...
			echo "<input class=\"unconfirm_checkall\" title=\"" . __('Non confermare questo indirizzo email', 'subscribe2') . "\" type=\"checkbox\" name=\"unconfirm[]\" value=\"" . $subscriber . "\" /></td>\r\n";
			echo "<td style=\"text-align: center;\">\r\n";
			echo "<input class=\"delete_checkall\" title=\"" . __('Cancella questo indirizzo email', 'subscribe2') . "\" type=\"checkbox\" name=\"delete[]\" value=\"" . $subscriber . "\" />\r\n";
			echo "</td>\r\n";
			...
			echo "<input class=\"confirm_checkall\" title=\"" . __('Conferma questo indirizzo email', 'subscribe2') . "\" type=\"checkbox\" name=\"confirm[]\" value=\"" . $subscriber . "\" /></td>\r\n";
	...
	if ( $_POST['searchterm'] ) {
		echo "<tr><td colspan=\"6\" style=\"text-align: center;\"><b>" . __('Iscritti corrispondenti non trovati', 'subscribe2') . "</b></td></tr>\r\n";
	} else {
		echo "<tr><td colspan=\"6\" style=\"text-align: center;\"><b>" . __('NESSUNO', 'subscribe2') . "</b></td></tr>\r\n";
	}
}
if ( !empty($subscribers) ) {
	echo "<tr class=\"$alternate\"><td colspan=\"3\" style=\"text-align: center;\"><input type=\"submit\" class=\"button-secondary\" name=\"process\" value=\"" . __('Elabora', 'subscribe2') . "\" /></td>\r\n";
...
if ( in_array($what, array('registered', 'all_users')) || is_numeric($what) ) {
	...
	echo "<h2>" . __('Gestione di massa', 'subscribe2') . "</h2>\r\n";
	if ( $this->subscribe2_options['email_freq'] == 'never' ) {
		echo __('Le preferenze per gli utenti registrati selezionati nel filtro sopra possono essere cambiati in questa sezione.', 'subscribe2') . "<br />\r\n";
		echo "<strong><em style=\"color: red\">" . __('Considera che i cambiamenti alla privacy dell\'utente non possono essere annullati', 'subscribe2') . "</em></strong><br />\r\n";
		echo "<br />" . __('Azione da eseguire', 'subscribe2') . ":\r\n";
		echo "<label><input type=\"radio\" name=\"manage\" value=\"subscribe\" checked=\"checked\" /> " . __('Iscriviti', 'subscribe2') . "</label>&nbsp;&nbsp;\r\n";
		echo "<label><input type=\"radio\" name=\"manage\" value=\"unsubscribe\" /> " . __('Disinscriviti', 'subscribe2') . "</label><br /><br />\r\n";
		...
		echo "<br />" . __('Invia un\'email come', 'subscribe2') . ":\r\n";
		echo "<label><input type=\"radio\" name=\"format\" value=\"html\" /> " . __('HTML - Full', 'subscribe2') . "</label>&nbsp;&nbsp;\r\n";
		echo "<label><input type=\"radio\" name=\"format\" value=\"html_excerpt\" /> " . __('HTML - Estratto', 'subscribe2') . "</label>&nbsp;&nbsp;\r\n";
		echo "<label><input type=\"radio\" name=\"format\" value=\"post\" /> " . __('Testo normale - Full', 'subscribe2') . "</label>&nbsp;&nbsp;\r\n";
		echo "<label><input type=\"radio\" name=\"format\" value=\"excerpt\" checked=\"checked\" /> " . __('Testo normale - Estratto', 'subscribe2') . "</label>\r\n";
		echo "<p class=\"submit\"><input type=\"submit\" class=\"button-primary\" name=\"sub_format\" value=\"" . __('Aggiornamento di massa del Formato', 'subscribe2') . "\" /></p>";
	} else {
		echo __('Le preferenze per gli utenti registrati selezionati nel filtro sopra possono essere cambiati in questa sezione.', 'subscribe2') . "<br />\r\n";
		echo "<strong><em style=\"color: red\">" . __('Considera che i cambiamenti alla privacy dell\'utente non possono essere annullati', 'subscribe2') . "</em></strong><br />\r\n";
		echo "<br />" . __('Iscrivi gli utenti seguenti selezionati affinche possano ricevere emails di sintesi', 'subscribe2') . ":\r\n";
		...
		echo "<p class=\"submit\"><input type=\"submit\" class=\"button-primary\" name=\"sub_digest\" value=\"" . __('Aggiornamento di massa all\'iscrizione alle sintesi', 'subscribe2') . "\" /></p>";
	}
	echo "</div>\r\n";
}
echo "</form></div>\r\n";
