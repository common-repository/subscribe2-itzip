...
if ( isset($_POST['s2_admin']) && 'user' == $_POST['s2_admin'] ) {
	...
	echo "<div id=\"message\" class=\"updated fade\"><p><strong>" . __('Preferenze di iscrizine aggiornate.', 'subscribe2') . "</strong></p></div>\n";
}

// show our form
echo "<div class=\"wrap\">";
echo "<div id=\"icon-users\" class=\"icon32\"></div>";
echo "<h2>" . __('Impostazione delle notifiche', 'subscribe2') . "</h2>\r\n";
if ( isset($_GET['email']) ) {
	$user = get_userdata($user_ID);
	echo "<span style=\"color: red;line-height: 300%;\">" . __('Scelta delle preferenze di subscibe2 per gli utenti', 'subscribe2') . ": " . $user->display_name . "</span>";
}
...
if ( $this->subscribe2_options['email_freq'] == 'never' ) {
	echo __('Ricevi email come', 'subscribe2') . ": &nbsp;&nbsp;";
	...
	echo " " . __('Semplice testo - Full', 'subscribe2') . "</label>&nbsp;&nbsp;";
	...
	echo " " . __('Semplice testo - Estratto', 'subscribe2') . "</label><br /><br />\r\n";

	if ( $this->subscribe2_options['show_autosub'] == 'yes' ) {
		echo __('Iscrivimi automaticamente alle nuove categorie create', 'subscribe2') . ': &nbsp;&nbsp;';
	...
	if ( $this->subscribe2_options['one_click_profile'] == 'yes' ) {
		...
		echo "<h2>" . __('Iscrizione con un click / Disincrizione', 'subscribe2') . "</h2>\r\n";
		echo "<p class=\"submit\"><input type=\"submit\" class=\"button-primary\" name=\"subscribe\" value=\"" . __("Iscriviti a tutto", 'subscribe2') . "\" />&nbsp;&nbsp;";
		echo "<input type=\"submit\" class=\"button-primary\" name=\"unsubscribe\" value=\"" . __("Disinscriviti da tutto", 'subscribe2') . "\" /></p>";
	}
	...
	if ( $this->s2_mu ) {
		...
		if ( !empty($subscribed) ) {
			...
			echo "<p><a href=\"". $unsubscribe_link ."\" class=\"button\">" . __('Anulla la mia iscrizione a questo blog', 'subscribe2') . "</a></p>";
		} else {
			// else we show a "subscribe" link
			$subscribe_link = esc_url( add_query_arg('s2mu_subscribe', $blog_id) );
			echo "<p><a href=\"". $subscribe_link ."\" class=\"button\">" . __('Iscrivimi a tutte le categorie', 'subscribe2') . "</a></p>";
		}
		echo "<h2>" . __('Iscrizione alle categorie il', 'subscribe2') . " " . get_option('blogname') . " </h2>\r\n";
	} else {
		echo "<h2>" . __('Categorie a cui sono iscritto', 'subscribe2') . "</h2>\r\n";
	}
	...
	echo __('Vuoi ricevere sommari di nuovi posts?', 'subscribe2') . ': &nbsp;&nbsp;';
	echo "<label>";
	...
	echo " /> " . __('Si', 'subscribe2') . "</label> <label><input type=\"radio\" name=\"category\" value=\"-1\" ";
	if ( !get_user_meta($user_ID, $this->get_usermeta_keyname('s2_subscribed'), true) ) {
		echo " checked=\"checked\"";
	}
...
if ( count($this->get_authors()) > 1 && $this->subscribe2_options['email_freq'] == 'never' ) {
	...
	echo "<h2>" . __('Non inviare notifiche per articoli creati da questi autori', 'subscribe2') . "</h2>\r\n";
	$this->display_author_form(explode(',', get_user_meta($user_ID, $this->get_usermeta_keyname('s2_authors'), true)));
	echo "</div>\r\n";
}

// submit
echo "<p class=\"submit\"><input type=\"submit\" class=\"button-primary\" name=\"submit\" value=\"" . __("Aggiorna preferenze", 'subscribe2') . " &raquo;\" /></p>";
echo "</form>\r\n";
...
if ( $this->s2_mu && !isset($_GET['email']) ) {
	...
	if ( !empty($blogs_subscribed) ) {
		...
		echo "<h2>" . __('Blog ai quali si è iscritti', 'subscribe2') . "</h2>\r\n";
		...
		foreach ( $blogs_subscribed as $blog ) {
			...
			if ( $s2blog_id == $blog['blog_id'] ) {
				echo "<span class=\"buttons\">" . __('Visualizzazione delle preferenze ora', 'subscribe2') . "</span>\r\n";
			} else {
				echo "<span class=\"buttons\">";
				if ( is_user_member_of_blog($current_user->id, $blog['blog_id']) ) {
					echo "<a href=\"". $blog['subscribe_page'] . "\">" . __('Visualizza le preferenze', 'subscribe2') . "</a>\r\n";
				}
				echo "<a href=\"" . esc_url( add_query_arg('s2mu_unsubscribe', $blog['blog_id']) ) . "\">" . __('Anulla iscrizione', 'subscribe2') . "</a></span>\r\n";
			}
	...
	if ( !empty($blogs_notsubscribed) ) {
		...
		echo "<h2>" . __('Iscriviti a nuovi blogs', 'subscribe2') . "</h2>\r\n";
		echo "<ul class=\"s2_blogs\">";
		foreach ( $blogs_notsubscribed as $blog ) {
			...
			if ( $s2blog_id == $blog['blog_id'] ) {
				...
				echo "<a href=\"" . esc_url( add_query_arg('s2mu_subscribe', $blog['blog_id']) ) . "\">" . __('Iscriviti', 'subscribe2') . "</a></span>\r\n";
			}
			echo "<div class=\"additional_info\">" . $blog['description'] . "</div>\r\n";
			echo "</li>";
		}
