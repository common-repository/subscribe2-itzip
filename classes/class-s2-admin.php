...
/* ===== WordPress menu registration and scripts ===== */
	...
	*/
	function admin_menu() {
		...
		$s2user = add_submenu_page('s2', __('Your Subscriptions', 'subscribe2'), __('Le tue iscrizioni', 'subscribe2'), apply_filters('s2_capability', "read", 'user'), 's2', array(&$this, 'user_menu'), S2URL . 'include/email_edit.png');
		add_action("admin_print_scripts-$s2user", array(&$this, 'checkbox_form_js'));
		add_action("admin_print_styles-$s2user", array(&$this, 'user_admin_css'));

		$s2subscribers = add_submenu_page('s2', __('Iscritti', 'subscribe2'), __('Iscritti', 'subscribe2'), apply_filters('s2_capability', "manage_options", 'manage'), 's2_tools', array(&$this, 'subscribers_menu'));
		add_action("admin_print_scripts-$s2subscribers", array(&$this, 'checkbox_form_js'));

		$s2settings = add_submenu_page('s2', __('Impostazioni', 'subscribe2'), __('Impostazioni', 'subscribe2'), apply_filters('s2_capability', "manage_options", 'settings'), 's2_settings', array(&$this, 'settings_menu'));
		...
		add_submenu_page('s2', __('Invia Email', 'subscribe2'), __('Invia email', 'subscribe2'), apply_filters('s2_capability', "publish_posts", 'send'), 's2_posts', array(&$this, 'write_menu'));
...
/* ===== meta box functions to allow per-post override ===== */
	...
	*/
	function s2_meta_init() {
		add_meta_box('subscribe2', __('Ignora la notifica di iscrizione', 'subscribe2' ), array(&$this, 's2_meta_box'), 'post', 'advanced');
		add_meta_box('subscribe2', __('Ignora la notifica di iscrizione', 'subscribe2' ), array(&$this, 's2_meta_box'), 'page', 'advanced');
	} // end s2_meta_init()
	...
	*/
	function s2_meta_box() {
		...
		echo __("Clicca qui per disabilitare l'invio di email di notifica per questo articolo/pagina", 'subscribe2');
...
/* ===== WordPress menu helper functions ===== */
	...
	*/
	function display_category_form($selected = array(), $override = 1, $compulsory = array(), $name = 'category') {
		...
		echo "<label><input type=\"checkbox\" name=\"checkall\" value=\"checkall_" . $name . "\" /> " . __('Seleziona / Deseleziona tutto', 'subscribe2') . "</label>\r\n";
		echo "</td></tr>\r\n";
	...
	*/
	function display_format_form($formats, $selected = array()) {
		...
		echo "<table style=\"width: 100%; border-collapse: separate; border-spacing: 2px; *border-collapse: expression('separate', cellSpacing = '2px');\" class=\"editform\">\r\n";
		echo "<tr><td style=\"text-align: left;\" colspan=\"2\">\r\n";
		echo "<label><input type=\"checkbox\" name=\"checkall\" value=\"checkall_format\" /> " . __('Seleziona / Deseleziona Tutto', 'subscribe2') . "</label>\r\n";
		echo "</td></tr>\r\n";
		echo "<tr style=\"vertical-align: top;\"><td style=\"width: 50%; text-align: left\">\r\n";
	...
	*/
	function display_author_form($selected = array()) {
		...
		echo "<table style=\"width: 100%; border-collapse: separate; border-spacing: 2px; *border-collapse: expression('separate', cellSpacing = '2px');\" class=\"editform\">\r\n";
		echo "<tr><td style=\"text-align: left;\" colspan=\"2\">\r\n";
		echo "<label><input type=\"checkbox\" name=\"checkall\" value=\"checkall_author\" /> " . __('Seleziona / Deseleziona tutto', 'subscribe2') . "</label>\r\n";
		echo "</td></tr>\r\n";
	...
	*/
	function display_subscriber_dropdown($selected = 'registered', $submit = '', $exclude = array()) {
		...
		$who = array('all' => __('Tutti gli utenti e iscritti', 'subscribe2'),
			'public' => __('Iscritti pubblici', 'subscribe2'),
			'confirmed' => ' &nbsp;&nbsp;' . __('Confermato', 'subscribe2'),
			'unconfirmed' => ' &nbsp;&nbsp;' . __('Non confermato', 'subscribe2'),
			'all_users' => __('Tutti gli utenti registrati', 'subscribe2'),
			'registered' => __('Utenti registrati', 'subscribe2'));
	...
	*/
	function admin_dropdown($inc_author = false) {
		...
		if ( $inc_author ) {
			$author[] = (object)array('ID' => 'author', 'display_name' => __('Autore dell\'articolo', 'subscribe2'));
	...
	*/
	function display_digest_choices() {
		...
		if ( !is_readable($cron_file) ) {
			echo "<strong><em style=\"color: red\">" . __('Le funzioni cron di Wordpress possono essere disabilitate su questo server. La notifica di sintesi può non funzionare.', 'subscribe2') . "</em></strong><br />\r\n";
		}
		$scheduled_time = wp_next_scheduled('s2_digest_cron');
		$offset = get_option('gmt_offset') * 60 * 60;
		$schedule = (array)wp_get_schedules();
		$schedule = array_merge(array('never' => array('interval' => 0, 'display' => __('Per ogni articolo', 'subscribe2'))), $schedule);
		...
		echo "<br />" . __('Invia una notifica di sintesi all\'ora UTC', 'subscribe2') . ": \r\n";
		...
		echo "</select>\r\n";
		echo "<strong><em style=\"color: red\">" . __('L\'ora scelta sarà programmata a un data futura in relazione all\'ora UTC corrente', 'subscribe2') . "</em></strong>\r\n";
		if ( $scheduled_time ) {
			$datetime = get_option('date_format') . ' @ ' . get_option('time_format');
			echo "<p>" . __('L\'ora corrente UTC è', 'subscribe2') . ": \r\n";
			echo "<strong>" . date_i18n($datetime, false, 'gmt') . "</strong></p>\r\n";
			echo "<p>" . __('L\'ora corrente del blog è', 'subscribe2') . ": \r\n";
			echo "<strong>" . date_i18n($datetime) . "</strong></p>\r\n";
			echo "<p>" . __('La prossima notifica email quando l\'ora del tuo blog sarà successiva a', 'subscribe2') . ": \r\n";
			echo "<strong>" . date_i18n($datetime, $scheduled_time + $offset) . "</strong></p>\r\n";
			if ( !empty($this->subscribe2_options['previous_s2cron']) ) {
				echo "<p>" . __('Tentativo di reinvio dell\'emai di sintesi', 'subscribe2') . ": ";
				echo "<input type=\"submit\" class=\"button-secondary\" name=\"resend\" value=\"" . __('Reinvio di sintesi', 'subscribe2') . "\" /></p>\r\n";
			}
		} else {
			echo "<br />";
		}
...
/* ===== functions to show & handle one-click subscription ===== */
	...
	*/
	function one_click_profile_form($user) {
		echo "<h3>" . __('Iscrizione notifica email', 'subscribe2') . "</h3>\r\n";
		echo "<table class=\"form-table\">\r\n";
		echo "<tr><th scope=\"row\">" . __('Iscriviti / Annulla iscrizione', 'subscribe2') . "</th>\r\n";
		...
		echo "<span class=\"description\">" . __('Seleziona se vuoi ricevere notifica email quando nuovi articoli vengono pubblicati', 'subscribe2') . "</span>\r\n";
		echo "</td></tr></table>\r\n";
