...
// variables and constructor are declared at the end
	...
	*/
	function load_strings() {
		...
		$this->confirmation_sent = "<p class=\"s2_message\">" . __('Un messaggio di conferma è stato inviato alla tua casella di posta!', 'subscribe2') . "</p>";

		$this->already_subscribed = "<p class=\"s2_error\">" . __('Questo indirizzo email esiste già.', 'subscribe2') . "</p>";

		$this->not_subscribed = "<p class=\"s2_error\">" . __('Nessuna iscrizione con questo indirizzo email.', 'subscribe2') . "</p>";

		$this->not_an_email = "<p class=\"s2_error\">" . __('Spiacente, ma questo non sembra essere un indirizzo email.', 'subscribe2') . "</p>";

		$this->barred_domain = "<p class=\"s2_error\">" . __('Spiacente gli indirizzi email a questo dominio sono attualmente bloccati a causa di spam. Prego usa un altro indirizzo email.', 'subscribe2') . "</p>";

		$this->error = "<p class=\"s2_error\">" . __('Spiacente, sembra esserci un errore sul server. Ritenta più tardi.', 'subscribe2') . "</p>";

		$this->no_page = __('Perchè questo plugin funzioni correttamente devi abilitare il widget di subscribe2 due (sotto)', 'subscribe2');

		$this->mail_sent = "<p class=\"s2_message\">" . __('Messaggio inviato!', 'subscribe2') . "</p>";

		$this->mail_failed = "<p class=\"s2_error\">" . __('Fallito l\'invio del messaggio! Controlla le tue impostazioni e anche il tuo provider', 'subscribe2') . "</p>";

		// confirmation messages
		$this->no_such_email = "<p class=\"s2_error\">" . __('Questo indirizzo email non è registrato.', 'subscribe2') . "</p>";

		$this->added = "<p class=\"s2_message\">" . __('Ti sei iscritto con successo!', 'subscribe2') . "</p>";

		$this->deleted = "<p class=\"s2_message\">" . __('La cancellazione è andata a buon fine.', 'subscribe2') . "</p>";

		$this->subscribe = __('iscriviti', 'subscribe2'); //ACTION replacement in subscribing confirmation email

		$this->unsubscribe = __('cancella iscrizione', 'subscribe2'); //ACTION replacement in unsubscribing in confirmation email

		// menu strings
		$this->options_saved = __('Opzioni salvate!', 'subscribe2');
		$this->options_reset = __('Opzioni reimpostate!', 'subscribe2');
	} // end load_strings()
	...
	function prepare_export( $subscribers ) {
		...
		foreach ( $subscribers as $subscriber ) {
			if ( $this->is_registered($subscriber) ) {
				...
				$exportcsv .= __('Utente registrato', 'subscribe2');
				$exportcsv .= ',' . $user_info->display_name;
				$exportcsv .= $subscribed_cats . "\r\n";
			} else {
				if ( $this->is_public($subscriber) === '1' ) {
					$exportcsv .= $subscriber . ',' . __('Utente pubblico confermato', 'subscribe2') . "\r\n";
				} elseif ( $this->is_public($subscriber) === '0' ) {
					$exportcsv .= $subscriber . ',' . __('Utente pubblico non confermato', 'subscribe2') . "\r\n";
				}
			}
		}
	...
	*/
	function register_form() {
		...
		if ( 'wpreg' == $this->subscribe2_options['autosub'] ) {
			echo "<p>\r\n<label>";
			echo __('Controlla qui per iscriverti alle notifiche email per nuovi articoli', 'subscribe2') . ":<br />\r\n";
			...
			echo "<p>\r\n<center>\r\n";
			echo __('Regostrandoti a questo blog accetti di ricevere notifica di nuovi ma puoi disincriverti in ogni momento', 'subscribe2') . ".<br />\r\n";
			echo "</center></p>\r\n";
		}
	} // end register_form()
...
/* ===== wp-cron functions ===== */
	...
	*/
	function add_weekly_sched($sched) {
		$sched['weekly'] = array('interval' => 604800, 'display' => __('Settimanalmente', 'subscribe2'));
		return $sched;
	...
	*/
	function subscribe2_cron($preview = '', $resend = '') {
		...
		foreach ( $posts as $post ) {
			...
			if ( strstr($mailtext, "{AUTHORNAME}") ) {
				...
				if ( $author->display_name != '' ) {
					$message_post .= " (" . __('Autore', 'subscribe2') . ": " . html_entity_decode(apply_filters('the_author', $author->display_name), ENT_QUOTES) . ")";
					$message_posttime .= " (" . __('Autore', 'subscribe2') . ": " . html_entity_decode(apply_filters('the_author', $author->display_name), ENT_QUOTES) . ")";
				}
			}
			$message_post .= "\r\n";
			$message_posttime .= "\r\n";
			...
			$message_posttime .= __('Pubblicato il', 'subscribe2') . ": " . mysql2date($datetime, $post->post_date) . "\r\n";
			...
			if ( strstr($mailtext, "{CATS}") ) {
				...
				$message_post .= __('Pubblicato in', 'subscribe2') . ": " . $post_cat_names . "\r\n";
				$message_posttime .= __('Pubblicato in', 'subscribe2') . ": " . $post_cat_names . "\r\n";
			}
			if ( strstr($mailtext, "{TAGS}") ) {
				$post_tag_names = implode(', ', wp_get_post_tags($post->ID, array('fields' => 'names')));
				if ( $post_tag_names != '' ) {
					$message_post .= __('Taggato come', 'subscribe2') . ": " . $post_tag_names . "\r\n";
					$message_posttime .= __('Taggato come', 'subscribe2') . ": " . $post_tag_names . "\r\n";
				}
			}
			$message_post .= "\r\n";
			$message_posttime .= "\r\n";
		...
		$subject .= $display . " " . __('Email di sintesi', 'subscribe2');
		...
		if ( $preview != '' ) {
			$this->myemail = $preview;
			$this->myname = __('Anteprima di sintesi', 'subscribe2');
