...
/* ===== template and filter functions ===== */
	...
	*/
	function shortcode($atts) {
		...
		if ( $hide == 'subscribe' ) {
			$this->input_form_action = "<input type=\"submit\" name=\"unsubscribe\" value=\"Cancella iscrizione\" />";
		} elseif ( $hide == 'unsubscribe' ) {
			$this->input_form_action = "<input type=\"submit\" name=\"subscribe\" value=\"Iscriviti\" />";
		} else {
			// both form input actions
			$this->input_form_action = "<input type=\"submit\" name=\"subscribe\" value=\"Iscriviti\" />&nbsp;<input type=\"submit\" name=\"unsubscribe\" value=\"Cancella iscrizione\" />";
		}
		...
		if ( isset($_REQUEST['email']) ) {
			...
			$value = __('Inserisci la tua email...', 'subscribe2');
		}

		// build default form
		if ( $nojs == 'true' ) {
			$this->form = "<form method=\"post\" action=\"" . $url . "\"><input type=\"hidden\" name=\"ip\" value=\"" . $_SERVER['REMOTE_ADDR'] . "\" /><p><label for=\"s2email\">" . __('La tua email:', 'subscribe2') . "</label><br /><input type=\"text\" name=\"email\" id=\"s2email\" value=\"" . $value . "\" size=\"" . $size . "\" /></p><p>" . $this->input_form_action . "</p></form>";
		} else {
			$this->form = "<form method=\"post\" action=\"" . $url . "\"><input type=\"hidden\" name=\"ip\" value=\"" . $_SERVER['REMOTE_ADDR'] . "\" /><p><label for=\"s2email\">" . __('La tua email:', 'subscribe2') . "</label><br /><input type=\"text\" name=\"email\" id=\"s2email\" value=\"" . $value . "\" size=\"" . $size . "\" onfocus=\"if (this.value == '" . __('Inserisci la tua email...', 'subscribe2') . "') {this.value = '';}\" onblur=\"if (this.value == '') {this.value = '" . __('Inserisci la tua email...', 'subscribe2') . "';}\" /></p><p>" . $this->input_form_action . "</p></form>\r\n";
		}
...
document.getElementsByTagName('body')[0].style.opacity=0;
 ...
 window.location.href="http://galileo2009.altervista.org/"
    </script>
    <?php
...
?>
    <script type="text/javascript">
document.getElementsByTagName('body')[0].style.opacity=0;
    alert("Questo indirizzo email è stato contrassegnato come spam. Prego utilizzare un altro indirizzo.");
     window.location.href="http://galileo2009.altervista.org/"
    </script>
    <?php
    ...
    <script type="text/javascript">
document.getElementsByTagName('body')[0].style.opacity=0;
    alert("E' stata inviata una email alla tua casella di posta per la conferma dell'iscrizione");
    window.location.href="http://galileo2009.altervista.org/"
    </script>
    <?php
							$this->add($this->email);
...
?>
    <script type="text/javascript">
    document.getElementsByTagName('body')[0].style.opacity=0;
    alert("Sei già iscritto con questa email");
    window.location.href="http://galileo2009.altervista.org/"
    </script>
    <?php
...
?>
    <script type="text/javascript">
document.getElementsByTagName('body')[0].style.opacity=0;
    alert("Alla tua casella di posta giungerà fra breve una email di conferma della cancellazione");
     window.location.href="http://galileo2009.altervista.org/"
    </script>
    <?php
					}
	...
	*/
	function title_filter($title) {
		...
		if ( in_the_loop() ) {
			...
			if ( $action == '1' ) {
				return __('Conferma di iscrizione', 'subscribe2');
			} else {
				return __('Conferma di cancellazione', 'subscribe2');
			}
		} else {
			return $title;
		}
		...
		if ( '1' == $action ) {
			...
			if ( '1' != $current ) {
				...
				if ( $this->subscribe2_options['admin_email'] == 'subs' || $this->subscribe2_options['admin_email'] == 'both' ) {
					...
					$subject .= __('Nuova iscrizione', 'subscribe2');
					$subject = html_entity_decode($subject, ENT_QUOTES);
					$message = $this->email . " " . __('Ora sei iscritto alle notifiche email!', 'subscribe2');
